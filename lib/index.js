...
async function deliverAlarmToIm(ctx, payload, options) {
  ...
  if (!botId || !targetId) {
    ctx.logger.warn("netxops alarm-im: enabled but imBotId/imTargetId empty — skip IM delivery (create a target in IM 投递设置 and paste botId+targetId)");
    return;
  }
...
function capabilityGroupsFromSettings(fields) {
  const src = fields ?? {};
  const commonInPreset = src.groupCommonInPreset !== undefined ? src.groupCommonInPreset !== false : src.groupManagedNeInPreset !== false;
  const commonPublic = src.groupCommonPublic === true || src.groupManagedNePublic === true;
  return {
    nms: {
  ...
  const publish = () => {
    publishing = publishing.then(async () => {
      ...
      if (!tokenConfigured) {
        ...
        ctx.logger.info("netxops: published connection → %s tokenConfigured=true alarmPush=%s dsh=%s im=%s public=[%s]", apiUrl, current.alarmPushEnabled === true, current.alarmDeliverDsh !== false, current.alarmDeliverIm === true, groupsForPlane(groups, "public").join(",") || "(none)");
      }
    }).catch((error) => {
