...
async function resolveKbPath(call, path, signal) {
  const result = await call(NETXOPS_RPC_CHANNEL2, KB_RESOLVE_ENDPOINT, { path }, signal);
  if (result !== null && typeof result === "object" && result.ok === true) {
    return asKbSnapshot(result.value);
  }
  return unconfiguredKbSnapshot();
...
class NetxopsCardController {
  ...
  browseKbRoot() {
    ...
    if (picker === undefined || this.kbBrowseInFlight)
      return;
...
function apply(ctx) {
  ...
  ctx.inject(["remote.directoryPicker"], (dpCtx) => {
    const picker = dpCtx.remote?.directoryPicker;
