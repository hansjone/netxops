...
function parseImCatalogOptionKey(value) {
...
function ImDeliveryPicker(props) {
  ...
  const selectedKey = props.botId && props.targetId ? imCatalogOptionKey(props.botId, props.targetId) : "";
  const matched = options.some((row) => imCatalogOptionKey(row.botId, row.targetId) === selectedKey);
  const [manual, setManual] = import_react.useState(() => !!(selectedKey && !matched));
  import_react.useEffect(() => {
    if (selectedKey && matched)
      setManual(false);
    else if (selectedKey && !matched)
      setManual(true);
  }, [selectedKey, matched]);
  ...
  return /* @__PURE__ */ jsx_runtime.jsxs(jsx_runtime.Fragment, {
    children: [
          ...
          /* @__PURE__ */ jsx_runtime.jsxs("select", {
            ...
            children: [
              /* @__PURE__ */ jsx_runtime.jsx("option", {
                value: "",
                children: options.length === 0 ? props.labels.none : `— ${props.labels.target} —`
              }),
              options.map((row) => /* @__PURE__ */ jsx_runtime.jsx("option", {
                value: imCatalogOptionKey(row.botId, row.targetId),
                children: `${row.name} · ${row.channel || "im"} · ${row.targetId}`
              }, imCatalogOptionKey(row.botId, row.targetId))),
              /* @__PURE__ */ jsx_runtime.jsx("option", {
                value: "__manual__",
                children: props.labels.manual
              })
            ]
          }),
      ...
      showManual ? /* @__PURE__ */ jsx_runtime.jsxs(jsx_runtime.Fragment, {
        children: [
        ...
        children: [
          ...
          /* @__PURE__ */ jsx_runtime.jsxs("div", {
            ...
            children: [
              ...
              /* @__PURE__ */ jsx_runtime.jsx("p", {
                className: "dsh-nx-hint",
                children: t("capabilityGroupsHint")
              }),
...
var en = {
  title: "Netx Ops",
  description: "netx API, bearer token, and capability groups (nms / common / topology).",
  apiUrl: "API URL",
  ...
  capabilityGroupsHint: "One group ↔ one skill (netx-nms / netx-common / netx-topology). Defaults: nms + common in preset; topology and all public off. Save, then open a new session. Other agents can mount dsh-netxops/tools-nms|common|topology.",
  ...
  groupNms: "nms — skill netx-nms (alarms / inventory / SQL)",
  groupCommon: "common — skill netx-common (managed CLI + findTopologyPaths)",
...
var zh = {
  title: "Netx Ops",
  description: "netx API、Bearer Token，以及能力组（nms / common / topology）。",
  apiUrl: "API 地址",
  apiUrlHint: "netx REST 根地址，例如 http://127.0.0.1:8890（告警订阅复用同一地址）",
  lang: "语言",
  langHint: "响应语言提示（zh / en）。",
  capabilityGroups: "能力组",
  capabilityGroupsHint: "一组一个 skill（netx-nms / netx-common / netx-topology）。默认：nms + common 进入预设；topology 与全部公开关闭。保存后请新开会话。其他 Agent 可挂 dsh-netxops/tools-nms|common|topology。",
  nmsProvider: "nms provider",
  nmsProviderHint: "厂商适配器 id。当前支持：zte-ume（REST 仍为 /v1/ume/*；模型工具名为 netx__*Nms*）。",
  groupNms: "nms — skill netx-nms（告警 / 库存 / SQL）",
  groupCommon: "common — skill netx-common（纳管 CLI + findTopologyPaths）",
