  ...
  const publish = () => {
    publishing = publishing.then(async () => {
      ...
      if (!tokenConfigured) {
        ...
        ctx.logger.info("netxops: published connection → %s tokenConfigured=true alarmPush=%s dsh=%s im=%s", apiUrl, current.alarmPushEnabled === true, current.alarmDeliverDsh !== false, current.alarmDeliverIm === true);
      }
    }).catch((error) => {
