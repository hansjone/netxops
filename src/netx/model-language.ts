...
export function replyInstruction(setting: string | undefined | null): string {
  ...
  const noGloss = normalized === 'en'
    ? 'Never add Chinese glosses or bilingual parentheses such as "communication failure (失联)" or "alarm (告警)"; use one English wording only.'
    : 'Never add English glosses or bilingual parentheses such as "失联 (communication failure)" or "告警 (alarm)"; use one Chinese wording only.'
  return [
...
export function replyReminder(setting: string | undefined | null): string {
  ...
  const meta = REPLY_META[normalized]
  const noGloss = normalized === 'en'
    ? 'No Chinese words or (中文) glosses in the answer.'
    : 'No English glosses like (English term) in the answer.'
  return [
