# Mirrored from netx/skills 鈥?do not long-edit here
...
Groups: **ops** (`netx-ops`) + **topology** (`netx-topology`). One group 鈫?one skill.

**Keep netx-ops pure netx** (alarms / inventory / CLI). Knowledge-base triage belongs in
operator-subset `_skills/kb-*` packs registered by the plugin 鈥?do **not** write KB linkage
back into `netx-ops`.
